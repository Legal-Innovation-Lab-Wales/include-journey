...
module TeamMembers
  ...
  class WellbeingAssessmentsController < TeamMembersApplicationController
    ...
    def create
      @wellbeing_assessment = current_team_member.wellbeing_assessments.create!(user: @user)
    ...
    def wba_params
      params.require(:wellbeing_assessment)
        .permit(@wellbeing_metrics.map { |metric| "wellbeing_metric_#{metric.id}" })
    end
    ...
    def wba_scores
      ...
      @wellbeing_metrics.each do |metric|
        ...
        @wellbeing_assessment.wba_scores
          .create!(wellbeing_metric: metric, value: value)
        total += value.to_f
      end
