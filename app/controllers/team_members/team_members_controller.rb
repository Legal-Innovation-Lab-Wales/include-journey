...
module TeamMembers
  ...
  class TeamMembersController < AdminApplicationController
    ...
    def reject
      ...
      flash = if @team_member.destroy
        {success: "#{@team_member.full_name} has been rejected"}
      else
        {error: "#{@team_member.full_name} could not be rejected"}
      end
      redirect_to team_members_path, flash: flash
    end

    # PUT /team_members/:id/admin
    def toggle_admin
      if @team_member.update(admin: !@team_member.admin?)
        success(@team_member.admin?, 'an admin')
      else
        failure('admin')
      end
    end
